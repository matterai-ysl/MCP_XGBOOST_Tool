#!/usr/bin/env python3
"""
Enhanced Academic Report Generator for XGBoost Models

This module generates comprehensive academic reports for XGBoost training experiments,
...
class AcademicReportGenerator:
    """Enhanced academic report generator for XGBoost models with detailed analysis"""
    
    def __init__(self):
        self.report_template = """# XGBoost Training Report
...
This report documents a comprehensive XGBoost training experiment conducted for academic research and reproducibility purposes. The experiment involved{hyperopt_text} cross-validated model training with detailed performance analysis, data validation, and feature importance evaluation.
...
### {arch_section_number}.1 XGBoost Configuration
...
"""
    ...
    def _get_hyperparameter_ranges(self) -> Dict[str, Any]:
        """Get the hyperparameter search ranges used in XGBoost optimization"""
        return {
            "n_estimators": {
                "range": [50, 150],
                "step": 10,
                "description": "Number of boosting rounds (trees) in the ensemble"
            },
            "max_depth": {
                "range": [1, 10],
                "step": 1,
                "description": "Maximum depth of each tree in the ensemble"
            },
            "learning_rate": {
                "range": [0.01, 0.3],
                "scale": "log",
            ...
            "colsample_bylevel": {
                "range": [0.6, 1.0],
                "scale": "linear",
                "description": "Fraction of features used for each level in each tree"
            },
            "reg_alpha": {
                "range": [1e-8, 10.0],
                "scale": "log",
                "description": "L1 regularization term on weights (Lasso regularization)"
            },
            "reg_lambda": {
                "range": [1e-8, 10.0],
                "scale": "log",
                "description": "L2 regularization term on weights (Ridge regularization)"
            },
            "min_child_weight": {
                "range": [1, 10],
                "step": 1,
                "description": "Minimum sum of instance weight needed in a child node"
            },
            "gamma": {
                "range": [1e-8, 10.0],
                "scale": "log",
    ...
    def _format_report(self, data: Dict[str, Any], timestamp: str, model_id: str, model_directory: Path) -> str:
        ...
        format_params = {
            ...
            'balance_implication': self._get_balance_implication(metadata),
            ...
            'data_validation_section': self._format_data_validation_section(data.get('data_validation_results'), metadata.get('task_type')),
    ...
    def _format_dataset_info(self, metadata: Dict[str, Any]) -> str:
        ...
        if 'data_source' in metadata["tool_input_details"]:
            lines.append(f"| Data File | `{metadata["tool_input_details"]['data_source']}` |")
        if 'data_shape' in metadata:
            shape = metadata['data_shape']
            lines.append(f"| Data Shape | {shape} |")
        if 'n_features' in metadata['data_shape']:
            lines.append(f"| Number of Features | {metadata['data_shape']['n_features']} |")
    ...
    def _format_preprocessing_section(self, metadata: Dict[str, Any]) -> str:
        ...
        if not preprocessing_applied:
            return """No scaling was applied to the features. XGBoost can handle raw numerical features, but scaling (like standardization) is often recommended to improve training speed and performance, especially when using regularization.

**Strategy**: Use original data characteristics.

**Rationale**:
- **Interpretability**: Results can be directly related to the original feature scales.
- **Algorithm Robustness**: XGBoost is less sensitive to feature scaling than distance-based algorithms. However, for optimal performance, especially with L1/L2 regularization, scaling is beneficial.
"""
        else:
...
"""
    ...
    def _format_hyperopt_section(self, hyperopt_results: Dict[str, Any]) -> str:
        ...
        for param, config in param_ranges.items():
            if 'range' in config:
                ...
                if 'step' in config:
                    ...
                    range_str += f" ({config['scale']} scale)"
            else:
        ...
        if best_params:
            lines.extend([
                f"- **Ensemble Size**: {best_params.get('n_estimators', 'N/A')} boosting rounds balances performance and computational efficiency",
                f"- **Tree Complexity**: Maximum depth of {best_params.get('max_depth', 'N/A')} controls model complexity and overfitting",
                f"- **Learning Rate**: {best_params.get('learning_rate', 'N/A')} provides optimal step size for gradient descent",
                f"- **Regularization**: L1={best_params.get('reg_alpha', 'N/A'):.2e}, L2={best_params.get('reg_lambda', 'N/A'):.2e} prevent overfitting",
                f"- **Sampling**: {best_params.get('subsample', 'N/A')} row sampling and {best_params.get('colsample_bytree', 'N/A')} column sampling for robustness"
            ])
        
        return '\n'.join(lines) if lines else "Hyperparameter optimization was not performed or results are unavailable."
...
XGBoost provides several metrics to evaluate feature importance:
...
"""
        ...
        # 特征重要性图
        if 'feature_importance_tree.png' in files.get('plots', []):
            section += """![Feature Importance](feature_importance_tree.png)

**File:** `feature_importance_tree.png`
...
"""
    ...
    def _format_model_config(self, metadata: Dict[str, Any]) -> str:
        ...
        for param in key_params:
            if param in model_params:
                lines.append(f"| {param.replace('_', ' ').title()} | {model_params[param]} |")
        
        lines.append("| Booster | gbtree (tree-based model) |")

        # 添加其他参数
        for param, value in model_params.items():
            if param not in key_params:
                # Truncate long values for readability in the table
                value_str = str(value)
                if len(value_str) > 50:
                    value_str = value_str[:47] + "..."
                lines.append(f"| {param.replace('_', ' ').title()} | {value_str} |")
    ...
    def _format_key_findings(self, results: Dict[str, Any], metadata: Dict[str, Any], has_hyperopt: bool) -> str:
        ...
        if 'cv_scores' in results:
            ...
            if isinstance(cv_scores, dict) and 'test_score' in cv_scores:
                scores = cv_scores['test_score']
                mean_score = np.mean(scores)
                findings.append(f"1. **Model Performance**: The XGBoost achieved a cross-validation score of {mean_score:.6f}")
    ...
    def _format_technical_implementation(self) -> str:
        ...
        return """- **Framework**: XGBoost for gradient boosting implementation, scikit-learn for pipeline integration.
...
- **Serialization**: Joblib or Pickle for model and preprocessor persistence."""
    ...
    def _format_data_validation_section(self, validation_results: Optional[Dict[str, Any]], task_type: Optional[str] = None) -> str:
...
"""
        ...
        for check in validation_checks:
            ...
            key_findings = self._extract_key_findings(check, task_type)

            validation_table += f"| {check_name} | {method} | {status} | {issues_count} | {key_findings} |\n"

        # Generate detailed analysis for each validation type
        detailed_analysis = self._generate_detailed_validation_analysis_academic(validation_results, task_type)
...
"""
    ...
    def _format_enhanced_feature_importance_section(self, 
        ...
        if feature_importance_data:
            # Check if data is in the direct format (feature_name -> metrics)
            if self._is_direct_feature_importance_format(feature_importance_data):
                # Generate comprehensive feature importance analysis
                lines.extend(self._format_direct_feature_importance_analysis(feature_importance_data))
            else:
                # Handle legacy format for backwards compatibility
                self._format_legacy_feature_importance_analysis(feature_importance_data, lines)
    ...
    def _is_direct_feature_importance_format(self, data: Dict[str, Any]) -> bool:
        """Check if the data is in direct format (feature_name -> metrics)"""
        if not data or not isinstance(data, dict):
            return False
        
        # Check if first level keys look like feature names and have expected metrics
        for key, value in data.items():
            if isinstance(value, dict):
                # Check if it has expected feature importance metrics
                expected_keys = {'weight', 'gain', 'cover', 'permutation_mean', 'permutation_std'}
                if any(metric in value for metric in expected_keys):
                    return True
        return False
    
    def _format_direct_feature_importance_analysis(self, feature_data: Dict[str, Any]) -> List[str]:
        """Format feature importance analysis for direct data format"""
        lines = []
        
        # Calculate total importance for percentage calculations
        total_gain = sum(metrics.get('gain', 0) for metrics in feature_data.values())
        total_weight = sum(metrics.get('weight', 0) for metrics in feature_data.values())
        
        # Sort features by different metrics
        features_by_gain = sorted(feature_data.items(), 
                                key=lambda x: x[1].get('gain', 0), reverse=True)
        features_by_permutation = sorted(feature_data.items(), 
                                       key=lambda x: x[1].get('permutation_mean', 0), reverse=True)
        
        # XGBoost Tree-based Importance Table
        lines.extend([
        ...
        for i, (feature_name, metrics) in enumerate(features_by_gain, 1):
            gain = metrics.get('gain', 0)
            weight = metrics.get('weight', 0)
            cover = metrics.get('cover', 0)
            gain_pct = (gain / total_gain * 100) if total_gain > 0 else 0
            weight_pct = (weight / total_weight * 100) if total_weight > 0 else 0
            
            lines.append(f"| {i} | `{feature_name}` | {gain:.4f} | {weight:.0f} | {cover:.2f} | {gain_pct:.1f}% | {weight_pct:.1f}% |")
        ...
        for i, (feature_name, metrics) in enumerate(features_by_permutation, 1):
            mean_importance = metrics.get('permutation_mean', 0)
            std_importance = metrics.get('permutation_std', 0)
            ...
            # Determine reliability based on coefficient of variation
            ...
            lines.append(f"| {i} | `{feature_name}` | {mean_importance:.4f} | {std_importance:.4f} | {ci_text} | {reliability} |")
        ...
        gain_rankings = {feature: i+1 for i, (feature, _) in enumerate(features_by_gain)}
        perm_rankings = {feature: i+1 for i, (feature, _) in enumerate(features_by_permutation)}
        
        for feature_name in feature_data.keys():
            gain_rank = gain_rankings.get(feature_name, 0)
            perm_rank = perm_rankings.get(feature_name, 0)
            rank_diff = abs(gain_rank - perm_rank)
            ...
            lines.append(f"| `{feature_name}` | {gain_rank} | {perm_rank} | {rank_diff} | {consistency} |")
        ...
        lines.extend([
            "**Statistical Summary:**",
            "",
            f"- **Total Features Analyzed**: {len(feature_data)}",
            f"- **Gain-based Top Feature**: `{features_by_gain[0][0]}` (Gain: {features_by_gain[0][1].get('gain', 0):.4f})",
            f"- **Permutation-based Top Feature**: `{features_by_permutation[0][0]}` (Importance: {features_by_permutation[0][1].get('permutation_mean', 0):.4f})",
            "",
            "**Method Reliability Assessment:**",
            f"- **Average Permutation Std**: {sum(m.get('permutation_std', 0) for m in feature_data.values()) / len(feature_data):.4f}",
            f"- **Method Agreement**: {'High' if gain_rankings[features_by_gain[0][0]] <= 2 and perm_rankings[features_by_permutation[0][0]] <= 2 else 'Moderate'}",
            ""
        ])
        
        return lines
    
    def _format_legacy_feature_importance_analysis(self, feature_importance_data: Dict[str, Any], lines: List[str]) -> None:
        """Handle legacy feature importance data format for backwards compatibility"""
    ...
    def _extract_key_findings(self, check: Dict[str, Any], task_type: Optional[str] = None) -> str:
        ...
        if check_name == 'feature_correlations':
            ...
            if task_type == 'classification':
                ...
                # For regression, 'sample_balance' refers to target distribution, not class ratio.
                return "Target distribution checked"
        else:
            issues = len(check.get('issues', []))
            return f"{issues} issues found" if issues > 0 else "No issues"
    
    def _generate_detailed_validation_analysis_academic(self, validation_results: Dict[str, Any], task_type: Optional[str] = None) -> str:
        ...
        for check in validation_results.get('validation_results', []):
            ...
            if check_name == 'feature_correlations':
                ...
                analysis.append(self._format_balance_analysis_academic(details, task_type))
    ...
    def _format_correlation_analysis_academic(self, details: Dict[str, Any]) -> str:
        ...
        threshold = details.get('correlation_threshold', 0.7)
        
        # Extract correlation matrices and target correlations
        correlation_matrices = details.get('correlation_matrices', {})
        target_correlations = details.get('target_correlations', {})
        
        # Generate detailed correlation analysis
        correlation_details = self._generate_detailed_correlation_tables(correlation_matrices, target_correlations, threshold)
        feature_classification = details.get('feature_classification', {})
        continuous_features = feature_classification.get('continuous', [])
        categorical_features = feature_classification.get('categorical', [])
        target_feature = feature_classification.get('target', '')
        continuous_features_str = ", ".join(continuous_features) if continuous_features else "None"
        categorical_features_str = ", ".join(categorical_features) if categorical_features else "None"
        target_feature_str = ", ".join(target_feature) if isinstance(target_feature, list) else target_feature
...
Continuous Features: {continuous_features_str}
Categorical Features: {categorical_features_str}
Target Feature: {target_feature_str}

**Statistical Findings**:
{correlation_details if correlation_details else "- No highly correlated features detected (|r| < " + str(threshold) + ")"}
...
**Impact Assessment**: {"High feature correlation may lead to multicollinearity issues and reduced model interpretability." if high_corr else "Low feature correlation indicates good feature independence, supporting model stability."}"""
    ...
    def _generate_detailed_correlation_tables(self, correlation_matrices: Dict[str, Any], target_correlations: Dict[str, Any], threshold: float) -> str:
        """Generate detailed correlation tables for academic report"""
        if not correlation_matrices and not target_correlations:
            return ""
        
        sections = []
        
        # 1. 连续-连续特征相关性分析 (Continuous-Continuous)
        if correlation_matrices.get('continuous_continuous'):
            continuous_data = correlation_matrices['continuous_continuous']
            corr_matrix = continuous_data.get('correlation_matrix', {})
            
            if corr_matrix.get('pearson'):
                pearson_table = self._format_feature_correlation_matrix(corr_matrix['pearson'], "Pearson")
                if pearson_table:
                    sections.append(f"**Continuous Features vs Continuous Features Correlation Analysis (Pearson Correlation Coefficient)**:\n\n{pearson_table}")
            
            if corr_matrix.get('spearman'):
                spearman_table = self._format_feature_correlation_matrix(corr_matrix['spearman'], "Spearman")  
                if spearman_table:
                    sections.append(f"**Continuous Features vs Continuous Features Correlation Analysis (Spearman's Rank Correlation)**:\n\n{spearman_table}")
        
        # 2. 离散-离散特征关联性分析 (Categorical-Categorical)
        if correlation_matrices.get('categorical_categorical'):
            categorical_data = correlation_matrices['categorical_categorical']
            corr_matrix = categorical_data.get('correlation_matrix', {})
            
            if corr_matrix:
                cramers_table = self._format_categorical_correlation_matrix(corr_matrix, "Cramér's V")
                if cramers_table:
                    sections.append(f"**Categorical Features vs Categorical Features Correlation Analysis (Cramér's V Coefficient)**:\n\n{cramers_table}")
        
        # 3. 连续-离散特征关联性分析 (Continuous-Categorical)
        if correlation_matrices.get('continuous_categorical'):
            mixed_data = correlation_matrices['continuous_categorical']
            corr_matrix = mixed_data.get('correlation_matrix', {})
            
            if corr_matrix:
                mixed_table = self._format_mixed_correlation_matrix(corr_matrix, "Correlation Ratio")
                if mixed_table:
                    sections.append(f"**Continuous Features vs Categorical Features Correlation Analysis (Correlation Ratio)**:\n\n{mixed_table}")
        
        # 4. 连续特征与目标变量相关性 (Continuous Feature-Target)
        if target_correlations.get('continuous_features'):
            target_table = self._format_target_correlation_table(target_correlations['continuous_features'])
            if target_table:
                sections.append(f"**Continuous Features vs Target Variable Correlation Analysis**:\n\n{target_table}")
        
        # 5. 离散特征与目标变量关联性 (Categorical Feature-Target)
        if target_correlations.get('categorical_features'):
            categorical_target_table = self._format_categorical_target_correlation_table(target_correlations['categorical_features'])
            if categorical_target_table:
                sections.append(f"**Categorical Features vs Target Variable Correlation Analysis**:\n\n{categorical_target_table}")
        
        return "\n\n".join(sections) if sections else ""
    
    def _format_feature_correlation_matrix(self, corr_matrix: Dict[str, Any], method: str) -> str:
        """Format feature correlation matrix as table"""
        if not corr_matrix:
            return ""
        ...
        # Create correlation pairs table (excluding self-correlations)
        table = "| Feature 1 | Feature 2 | Correlation | Absolute Value |\n|-----------|-----------|-------------|----------------|\n"
        
        correlation_pairs = []
        for i, feat1 in enumerate(features):
            for j, feat2 in enumerate(features):
                if i < j:  # Only upper triangle to avoid duplicates
                    corr_value = corr_matrix.get(feat1, {}).get(feat2, 0.0)
                    correlation_pairs.append((feat1, feat2, corr_value))
        
        # Sort by absolute correlation value (descending)
        correlation_pairs.sort(key=lambda x: abs(x[2]), reverse=True)
        
        # Add top correlations to table (limit to 10 pairs)
        for feat1, feat2, corr_value in correlation_pairs[:10]:
            abs_corr = abs(corr_value)
            table += f"| {feat1} | {feat2} | {corr_value:.4f} | {abs_corr:.4f} |\n"
        
        return table if correlation_pairs else ""
    
    def _format_target_correlation_table(self, target_corr: Dict[str, Any]) -> str:
        """Format feature-target correlation table"""
        if not target_corr:
            return ""
        
        table = "| Feature | Correlation | Method | Absolute Value | Strength |\n|---------|-------------|--------|----------------|----------|\n"
        
        # Sort features by absolute correlation value
        sorted_features = sorted(target_corr.items(), 
                               key=lambda x: abs(x[1].get('value', 0)), 
                               reverse=True)
        
        for feature, corr_data in sorted_features:
            corr_value = corr_data.get('value', 0.0)
            ...
            table += f"| {feature} | {corr_value:.4f} | {method} | {abs_corr:.4f} | {strength} |\n"
        
        return table
    
    def _format_categorical_correlation_matrix(self, corr_matrix: Dict[str, Any], method: str) -> str:
        """Format categorical correlation matrix (Cramér's V) as table"""
        if not corr_matrix:
            return ""
        ...
        # Create correlation pairs table (excluding self-correlations)
        table = "| Feature 1 | Feature 2 | Cramér's V | Absolute Value | Strength |\n|-----------|-----------|------------|----------------|----------|\n"
        ...
        # Add top correlations to table (limit to 15 pairs for categorical)
        for feat1, feat2, corr_value in correlation_pairs[:15]:
            abs_corr = abs(corr_value)
            
            # Determine association strength for Cramér's V
            if abs_corr >= 0.8:
            ...
            table += f"| {feat1} | {feat2} | {corr_value:.4f} | {abs_corr:.4f} | {strength} |\n"
        
        return table if correlation_pairs else ""
    
    def _format_mixed_correlation_matrix(self, corr_matrix: Dict[str, Any], method: str) -> str:
        """Format continuous-categorical correlation matrix (Correlation Ratio) as table"""
        if not corr_matrix:
            return ""
        
        # Extract continuous-categorical pairs
        table = "| Categorical Feature | Continuous Feature | Correlation Ratio | Absolute Value | Strength |\n|-------------------|-------------------|-------------------|----------------|----------|\n"
        
        correlation_pairs = []
        for cat_feature, cont_features in corr_matrix.items():
            if isinstance(cont_features, dict):
                for cont_feature, corr_value in cont_features.items():
                    correlation_pairs.append((cat_feature, cont_feature, corr_value))
        
        # Sort by absolute correlation value (descending)
        correlation_pairs.sort(key=lambda x: abs(x[2]), reverse=True)
        
        # Add correlations to table (limit to 20 pairs for mixed)
        for cat_feat, cont_feat, corr_value in correlation_pairs[:20]:
            abs_corr = abs(corr_value)
            # New strength thresholds (Correlation Ratio)
            if abs_corr > 0.14:
                strength = "Large effect (Strong association)"
            elif abs_corr > 0.06:
                strength = "Medium effect (Moderate association)"
            elif abs_corr > 0.01:
                strength = "Small effect (Weak association)"
            else:
                strength = "Negligible"
            table += f"| {cat_feat} | {cont_feat} | {corr_value:.4f} | {abs_corr:.4f} | {strength} |\n"
        
        return table if correlation_pairs else ""
    
    def _format_categorical_target_correlation_table(self, target_corr: Dict[str, Any]) -> str:
        """Format categorical feature-target correlation table"""
        if not target_corr:
            return ""
        
        table = "| Categorical Feature | Association | Method | Absolute Value | Strength |\n|------------------- |-------------|--------|----------------|----------|\n"
        ...
        for feature, corr_data in sorted_features:
            corr_value = corr_data.get('value', 0.0)
            method = corr_data.get('method', 'cramers_v')
            abs_corr = abs(corr_value)
            
            # Determine association strength for categorical features
            if abs_corr >= 0.8:
                strength = "Very Strong"
            elif abs_corr >= 0.6:
                strength = "Strong"
    ...
    def _format_correlation_table_academic(self, correlations: List[Dict]) -> str:
        """Format correlation table for academic report (legacy method kept for compatibility)"""
        if not correlations:
            return ""
    ...
    def _format_multicollinearity_analysis_academic(self, details: Dict[str, Any]) -> str:
        ...
        vif_scores = details.get('vif_scores', {})
        total_features = details.get('total_features_analyzed', len(vif_scores))
        
        # Generate detailed VIF table
        vif_table = self._generate_detailed_vif_table(vif_scores, vif_threshold) if vif_scores else ""
...
**Statistical Findings**:
{vif_table if vif_table else f"- All features showed VIF < {vif_threshold}, indicating acceptable multicollinearity levels"}
...
**Methodological Impact**: {"Elevated VIF scores suggest linear dependencies between predictors, which may compromise model stability and coefficient interpretation." if high_vif else "Low VIF scores support the assumption of feature independence required for many machine learning algorithms."}"""
    ...
    def _generate_detailed_vif_table(self, vif_scores: Dict[str, float], vif_threshold: float) -> str:
        """Generate detailed VIF table for academic report"""
        if not vif_scores:
            return ""
        
        # Sort features by VIF score (descending)
        sorted_features = sorted(vif_scores.items(), key=lambda x: x[1], reverse=True)
        
        table = "**VIF Scores for All Features**:\n\n"
        table += "| Feature | VIF Score | R² | Interpretation | Status |\n"
        table += "|---------|-----------|----|--------------|---------|\n"
        
        for feature, vif_score in sorted_features:
            # Calculate R² from VIF: R² = 1 - (1/VIF)
            r_squared = 1 - (1 / vif_score) if vif_score > 0 else 0
            
            # Determine interpretation
            if vif_score >= 10:
                interpretation = "Severe"
                status = "⚠️ HIGH"
            elif vif_score >= vif_threshold:
            ...
            table += f"| {feature} | {vif_score:.4f} | {r_squared:.4f} | {interpretation} | {status} |\n"
        
        return table
    
    def _format_vif_table_academic(self, vif_features: List[Dict]) -> str:
        """Format VIF table for academic report (legacy method kept for compatibility)"""
        if not vif_features:
            return ""
...
**Distribution Quality Impact**: {"Significant distribution anomalies may affect model performance and require preprocessing interventions." if issues else "Feature distributions meet statistical assumptions for machine learning applications."}"""
    ...
    def _format_continuous_features_academic(self, cont_dist: Dict[str, Any]) -> str:
        ...
        if not cont_dist or not isinstance(cont_dist, dict):
            return "No continuous features analyzed."

        normality_tests = cont_dist.get('normality_tests', {})
        skewness_analysis = cont_dist.get('skewness_analysis', {})
        outlier_detection = cont_dist.get('outlier_detection', {})
        identified_issues = cont_dist.get('identified_issues', {})
        distribution_statistics = cont_dist.get('distribution_statistics', {})
        feature_names = list(normality_tests.keys())
        if not feature_names:
            return "No continuous features with detailed analysis found."

        table = "| Feature | mean | std | min | max | max | median | Skewness | Kurtosis | Normality | Outliers (%) | Issues |\n|---------|----------|---------|----------|---------|----------|---------|----------|-----------|-------------|--------|\n"
        
        for feature in sorted(feature_names):
            norm_stats = normality_tests.get(feature, {})
            skew_stats = skewness_analysis.get(feature, {})
            outlier_stats = outlier_detection.get(feature, {})
            mean = distribution_statistics.get(feature, {}).get('mean', 0)
            std = distribution_statistics.get(feature, {}).get('std', 0)
            min = distribution_statistics.get(feature, {}).get('min', 0)
            max = distribution_statistics.get(feature, {}).get('max', 0)
            median = distribution_statistics.get(feature, {}).get('median', 0)
            skewness = skew_stats.get('value', skew_stats.get('skewness', 0))
            kurtosis = distribution_statistics.get(feature, {}).get('kurtosis', 0)
            # Safely get normality status from the 'shapiro' test, handling bool or string.
            norm_test_result = norm_stats.get('shapiro', {}).get('is_normal')
            if isinstance(norm_test_result, str):
                is_normal = norm_test_result.lower() == 'true'
            else:
                is_normal = bool(norm_test_result)
            
            is_normal_str = "Yes" if is_normal else "No"
            
            outlier_pct = outlier_stats.get('ratio', outlier_stats.get('outlier_percentage', 0)) * 100
            
            # Try to determine how many issues relate to this feature
            issues_root = cont_dist.get('issues', [])
            issues_count = sum(1 for msg in issues_root if feature in str(msg))
            
            table += f"| {feature} | {mean:.3f} | {std:.3f} | {min:.3f} | {max:.3f} | {max:.3f} | {median:.3f} | {skewness:.3f} | {kurtosis:.3f} | {is_normal_str} | {outlier_pct:.1f}% | {issues_count} |\n"

        issue_list = cont_dist.get('issues', [])
        if issue_list:
    ...
    def _format_balance_analysis_academic(self, details: Dict[str, Any], task_type: Optional[str] = None) -> str:
        """Format balance analysis for academic report"""
        if not details or not isinstance(details, dict):
            return ""

        if task_type == 'regression':
            stats = details.get('target_statistics', {})
            outlier_ratio = details.get('outlier_ratio', 0)
            ...
            if stats:
                table_lines.append(f"| Mean | {stats.get('mean', 0):.4f} |")
                table_lines.append(f"| Standard Deviation | {stats.get('std', 0):.4f} |")
                table_lines.append(f"| Minimum | {stats.get('min', 0):.4f} |")
                table_lines.append(f"| 25th Percentile | {stats.get('25th_percentile', 0):.4f} |")
                table_lines.append(f"| Median (50th) | {stats.get('50th_percentile', 0):.4f} |")
                table_lines.append(f"| 75th Percentile | {stats.get('75th_percentile', 0):.4f} |")
                table_lines.append(f"| Maximum | {stats.get('max', 0):.4f} |")
                table_lines.append(f"| Skewness | {skewness:.4f} |")
                table_lines.append(f"| Kurtosis | {kurtosis:.4f} |")
            
            table = '\n'.join(table_lines)
            
            return f"""#### 2.4.2.4 Target Variable Distribution Analysis
...
**Results**:
- **Outlier Ratio**: {outlier_ratio:.2%} of target values were identified as outliers using the IQR method.
- **Distribution Shape**: Skewness of {skewness:.3f} and Kurtosis of {kurtosis:.3f}.
...
**Methodological Implications**: The distribution of the target variable is crucial for regression model performance. Significant skewness or a high number of outliers may suggest the need for target transformation (e.g., log transformation) to improve model stability and accuracy."""
        ...
        else: # Default to classification behavior
...
| **Prediction Method** | {'Ensemble averaging of boosted trees' if task_type == 'regression' else 'Sum of weighted predictions'} |
...
|------|-------------|"""
        ...
        if results.get('confidence_scores'):
            ...
            if task_type == 'classification':
                ...
                md_content += "- **Primary Method**: Prediction variance across individual trees in XGBoost ensemble\n"
                ...
                md_content += "- Each tree in the XGBoost ensemble makes an independent prediction\n"
        ...
        md_content += f"*MCP XGBoost Tool - Prediction Experiment Report*\n"
...
"""
    ...
    def _get_balance_implication(self, metadata: Dict[str, Any]) -> str:
        task_type = metadata.get('task_type', 'regression')
        if task_type == 'classification':
            return "Class balance verified for classification task"
        else:
            return "Target variable distribution analyzed for regression task"